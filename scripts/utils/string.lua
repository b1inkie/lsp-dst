...
---EntityScript: 获取字符串的一个子串，支持 UTF-8 编码。
...
---EntityScript: 从字符串末尾开始查找第一次匹配的子字符串，并始终执行普通匹配。(非标准库)
...
---EntityScript: 从字符串末尾开始查找第一次匹配的模式。(非标准库)
...
---EntityScript: 根据指定的分隔符将字符串分割成多个子串。
...
---EntityScript: 转换字符串为小写形式，支持 UTF-8 编码。
---@param str string # 待转换的字符串
...
---EntityScript: 返回一个数组，包含字符串中所有匹配模式的起始和结束索引对。(非标准库)
---@param s string # 要搜索的字符串
---@param pattern string # 匹配模式
...
---EntityScript: 生成一个迭代器，用于遍历字符串中所有匹配给定模式的子串。
...
---EntityScript: 生成一个指定长度和字符集的随机字符串。(非标准库)
...
---EntityScript: 将数字转换为对应的 ASCII 字符。
...
---EntityScript: C库(非标准库)
---author: lan
function string:gfind()
end

---EntityScript: 检查字符串是否以指定前缀开始。(非标准库)
...
---EntityScript: 查找字符串中第一个符合模式的子串的位置。
...
---EntityScript: 从字符串中匹配第一个符合模式的子串。
---@param str string # 待匹配的字符串
---@param pattern string # 匹配模式
...
---EntityScript: 转换字符串为大写形式，支持 UTF-8 编码。
---@param str string # 待转换的字符串
---@return string # 转换后的大写字符串
---author: lan
...
---EntityScript: 计算字符串的 UTF-8 编码长度。
---@param str string # 待计算长度的字符串
